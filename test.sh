...
git clone https://github.com/KiSchnelle/role_cuda.git

cat << EOF >> /var/log/ansible/roles/role_lmod/defaults/main.yml
# if u want to change smth
EOF

cd /var/log/ansible
cp /var/log/ansible/roles/role_cuda/run.yml .
ansible-playbook run.yml --user=$ANSIBLE_USER --extra-vars "ansible_sudo_pass=$ANSIBLE_USER_PASSWORD" >> role_cuda_log.txt

# rm /var/log/ansible/roles/role_cuda/defaults/main.yml
